...
async fn test_vscode_tool_loading_scenario() -> Result<()> {
    ...
    assert!(build_output.status.success(), 
        "Release build failed: {}", String::from_utf8_lossy(&build_output.stderr));
    ...
    assert!(!stderr_content.contains("No valid tool configurations found"), 
        "Server failed to find tool configurations:\n{}", stderr_content);
    
    // This should contain the success message
    assert!(stderr_content.contains("Loaded") && stderr_content.contains("tool configurations"), 
...
async fn test_vscode_dev_tool_loading_scenario() -> Result<()> {
    ...
    let server_output = Command::new("cargo")
        .args(&["run", "--release", "--bin", "ahma_mcp", "--", "--server", "--tools-dir", "tools"])
        .current_dir(".") // Same as VS Code's workspaceFolder
        .env("RUST_LOG", "info")
        .output()?;
