...
mod realistic_endless_notification_test {
    ...
    async fn test_realistic_endless_notification_scenario() {
        ...
        let adapter = Arc::new(
            Adapter::new(operation_monitor.clone(), shell_pool_manager.clone()).unwrap()
        );
        ...
        let operation_id = adapter.execute_async_in_dir(
            ...
            Some(serde_json::Map::from_iter(vec![
                ("_subcommand".to_string(), serde_json::Value::String("version".to_string()))
            ])),
        ...
        loop {
            ...
            if !completed_ops.is_empty() {
                println!("✅ Operation completed after {} attempts", completion_wait_attempts + 1);
                break;
            }
            completion_wait_attempts += 1;
            if completion_wait_attempts > 50 { // 10 second timeout
        ...
        for iteration in 1..=6 {
            ...
            let found_operations: Vec<String> = completed_ops.iter()
                .map(|op| op.id.clone())
                .collect();
            ...
            if iteration > 1 && !found_operations.is_empty() {
                ...
                for op_id in &found_operations {
                    for (prev_iteration, prev_operations) in &notification_history[..notification_history.len()-1] {
                        if prev_operations.contains(op_id) {
                            println!("🐛 ENDLESS NOTIFICATION BUG DETECTED!");
                            println!("   Operation {} appeared in iteration {} and again in iteration {}", 
                ...
                if bug_detected {
                    ...
                    panic!("ENDLESS NOTIFICATION LOOP BUG REPRODUCED! This matches the user's log pattern.");
                }
            }
        ...
        for (iter, ops) in &notification_history {
            println!("   Iteration {}: {} operations - {:?}", iter, ops.len(), ops);
        }
        
        // The first iteration should find the operation, subsequent ones should be empty
        assert!(!notification_history[0].1.is_empty(), "First iteration should find completed operation");
    ...
    async fn test_multiple_operations_notification_behavior() {
        ...
        let adapter = Arc::new(
            Adapter::new(operation_monitor.clone(), shell_pool_manager).unwrap()
        );

        // Start multiple operations quickly
        let op1 = adapter.execute_async_in_dir("cargo", Some(serde_json::Map::from_iter(vec![
            ("_subcommand".to_string(), serde_json::Value::String("version".to_string()))
        ])), "/Users/paul/github/ahma_mcp", Some(30)).await;

        let op2 = adapter.execute_async_in_dir("cargo", Some(serde_json::Map::from_iter(vec![
            ("_subcommand".to_string(), serde_json::Value::String("--version".to_string()))
        ])), "/Users/paul/github/ahma_mcp", Some(30)).await;
        ...
        loop {
            ...
            if completion_attempts > 33 { // 10 second timeout
                let completed_ops = operation_monitor.get_completed_operations().await;
                if completed_ops.len() == 1 {
                    println!("⚠️  Only 1 operation completed, proceeding with test");
                    break;
                } else {
                    panic!("Operations never completed: {} completed", completed_ops.len());
                }
            }
        }
        ...
        for iteration in 1..=4 {
            ...
            println!("📊 Iteration {}: Found {} operations: {:?}", 
                   iteration, current_ops.len(), current_ops);

            // Check for duplicates across iterations
            for op_id in &current_ops {
                if all_seen_operations.contains(op_id) {
                    panic!("ENDLESS NOTIFICATION BUG: Operation {} seen again in iteration {}", op_id, iteration);
                }
