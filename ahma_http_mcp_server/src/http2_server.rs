...
use crate::{
    ...
    handler::{McpServerState, handle_mcp_post, health_check},
};
use axum::{
    Router,
    routing::{get, post},
...
pub async fn start_http2_server<H: ServerHandler + Send + Sync + Clone + 'static>(
    ...
    if config.enable_tls {
        ...
        let certs = load_certs_from_pem(&cert_pem).map_err(|e| ServerError::Certificate(e))?;

        let key = load_private_key_from_pem(&key_pem).map_err(|e| ServerError::Certificate(e))?;
        ...
        info!(
            "HTTP/2 server with TLS listening on https://{}",
            config.bind_addr
        );
        ...
        info!(
            "HTTP/2 server (plaintext) listening on http://{}",
            config.bind_addr
        );
