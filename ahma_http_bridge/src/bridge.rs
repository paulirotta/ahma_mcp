...
async fn handle_mcp_request(
    ...
    let sender = match &state.sender {
        Some(s) => s,
        None => {
            return error_response(-32603, "Internal error: no sender in single-process mode");
        }
    };
    ...
    if let Err(e) = sender.send(payload).await {
        ...
        return error_response(-32603, "Failed to send request to server process");
    }
    ...
    if let Some(rx) = response_rx {
        match tokio::time::timeout(Duration::from_secs(60), rx).await {
            ...
            Ok(Err(_)) => error_response(-32603, "Channel closed before response received"),
            Err(_) => {
                if let Some(id) = &id_opt {
                    state.pending_requests.remove(id);
                }
