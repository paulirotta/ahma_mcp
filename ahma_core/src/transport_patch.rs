        ...
        async move {
            ...
            loop {
                ...
                let message_body = if first_line.starts_with("Content-Length:") {
                    // Header Mode
                    let len_str = first_line.trim().strip_prefix("Content-Length:").unwrap_or("0").trim();
                    let content_len: usize = len_str.parse().unwrap_or(0);
                    eprintln!("[AhmaTransport] Detected Header Framing. Content-Length: {}", content_len);

                    // Skip remaining headers until empty line
                    loop {
                        let mut h = String::new();
                        if let Ok(n) = r.read_line(&mut h).await {
                            if n == 0 || h.trim().is_empty() { break; }
                        } else {
                            break;
                        }
                    }
                ...
                let mut value: Value = match serde_json::from_str(&message_body) {
                    ...
                    Err(e) => {
                        if !message_body.trim().is_empty() {
                            eprintln!("[AhmaTransport] Invalid JSON: {} | Content: {}", e, message_body);
                        }
                ...
                if let Some(method) = value.get("method").and_then(|v| v.as_str())
                    && method == "initialize" {
                        eprintln!("[AhmaTransport] Detected 'initialize' request. Checking capabilities...");
                        if let Some(params) = value.get_mut("params")
                            && let Some(caps) = params.get_mut("capabilities")
                                && let Some(tasks) = caps.get("tasks") {
                                    if tasks.is_object() {
                                        eprintln!("[AhmaTransport] Patching: Removing 'tasks' capability object");
                                        if let Some(caps_obj) = caps.as_object_mut() {
                                            caps_obj.remove("tasks");
                                        }
                                    } else {
                                        eprintln!("[AhmaTransport] 'tasks' capability found but not an object: {:?}", tasks);
                                    }
                                }
                    }
                // -----------------------
