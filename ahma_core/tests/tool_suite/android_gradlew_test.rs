...
mod android_tests {
    ...
    use ahma_core::test_utils::test_utils::{dir_exists, file_exists};
    use anyhow::Result;
    ...
    async fn test_gradlew_essential_subcommands() -> Result<()> {
        let workspace_dir = get_workspace_dir();
        let gradlew_tool_path = workspace_dir.join("ahma_core/examples/configs/gradlew.json");
        let tool_content = tokio::fs::read_to_string(&gradlew_tool_path).await?;
        let tool_def: Value = serde_json::from_str(&tool_content)?;
    ...
    async fn test_gradlew_async_sync_classification() -> Result<()> {
        ...
        for subcommand in subcommands {
            ...
            if sync_tasks.contains(&name) {
                assert!(
                    is_forced_sync,
                    "Task '{}' should be marked as synchronous to stay synchronous",
                    name
                );
                assert_eq!(
                    guidance_key,
                ...
                assert!(
                    !is_forced_sync,
                    "Task '{}' should NOT be synchronous (can run async)",
                    name
                );
                assert_eq!(
                    guidance_key,
